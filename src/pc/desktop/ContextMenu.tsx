...
export const ContextMenu: React.FC<ContextMenuProps> = ({
        ...
        >
          ...
          >
            ...
            {entry.submenu ? (
              <ChevronRight
                size={touch ? 16 : 13}
                className={`shrink-0 opacity-60 transition-transform ${touch && expanded ? "rotate-90" : ""}`}
              />
          ...
          {entry.submenu &&
            ...
            (touch ? (
              ...
              <Flyout>{entry.submenu.map((sub) => renderRow(sub, depth + 1))}</Flyout>
            ))}
        </div>
      </React.Fragment>
    );
  ...
  return (
    ...
    >
      {title && (
        <div
          className={`px-2.5 pb-1.5 pt-1 text-[11px] font-medium uppercase tracking-wider text-zinc-500 truncate ${touch ? "text-xs" : ""}`}
        >
          {title}
        </div>
      )}
