...
export const OkseSandbox: React.FC = () => {
  ...
  const handleInstantDecode = () => {
    ...
    sourceNibbles.forEach((_, i) => {
      ...
      logLines.push(
        `Glyph ${i}: Stem Orientation = ${bestOrient} (${bestOrient * 45}°), Circle = ${circleRes.present ? "ON" : "OFF"} [Nibble: 0x${nibble.toString(16).toUpperCase()}]`,
      );
    });
    ...
    logLines.push(
      matched
        ? "SUCCESS ✅ Round-trip exact match verified."
        : "FAIL ❌ Mismatch in round-trip conversion.",
    );
    setLogs(logLines);
  };
  ...
  const handleAnimateScan = () => {
    ...
    setScanDetails([]);
    setLogs([
      "Starting Optical Pixel Decoder scan...",
      "Shared context codebook: D4 Dihedral Symmetry (8 rotations x 2 mirror states) + Binary presence.",
    ]);
    ...
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto p-4 md:p-6 space-y-6 border-r border-zinc-800/60 max-w-full md:max-w-[65%] shrink-0">
        ...
        <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-5 space-y-3 shadow-sm">
          ...
          <p className="text-xs text-zinc-300 leading-relaxed font-sans">
            Conceived by Daigle, OKSE is a{" "}
            <strong>machine-decoded symbolic compression scheme</strong>. It takes advantage of the
            fact that <strong>b, d, p, q</strong> represent the exact same glyph (a circle + a stem)
            rotated and mirrored in 2D space. By anchoring an asymmetric ray at the center of each
            cell and checking 8 radial orientations, we compress 4 bits of binary state directly
            into a single mark (8 orientation states × 2 circle presence states = 16 values = 1
            nibble).
          </p>
        </div>
        ...
        <div className="bg-[#121216] border border-zinc-800/80 rounded-2xl p-4 shadow-inner space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] text-zinc-400 block font-sans">
              Enter Text String to Encapsulate into Marks:
            </label>
        ...
        <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-4 space-y-3">
          ...
          <p className="text-[11px] text-zinc-400 font-sans leading-relaxed">
            The test word <strong>qlipdoxb</strong> is a perfect pangram containing every core
            primitive in the language. It encodes in exactly 32 bits (4 bytes). Write it in the
            inputs above to trigger a full system validation!
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              ...
              >
                ...
                <span
                  className={`font-mono text-xs font-black px-1.5 py-0.5 rounded ${p.state ? "bg-indigo-600/30 text-indigo-400 border border-indigo-500/20" : "bg-zinc-900 text-zinc-700"}`}
                >
                  {p.char}
                </span>
              </div>
      ...
      <div className="flex-1 flex flex-col bg-[#050507] overflow-hidden max-h-[350px] md:max-h-none">
        ...
        <div className="p-4 border-b border-zinc-800 bg-zinc-950 shrink-0 space-y-3">
          ...
          {decodedResult ? (
            <div
              className={`p-3 rounded-xl border flex flex-col gap-1.5 animate-in slide-in-from-top-4 duration-300 ${
                isMatched
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[9px] uppercase font-black font-sans px-2 py-0.5 rounded ${
              ...
              <span className="text-[9px] text-zinc-400 font-sans leading-relaxed">
                The engine retrieved raw grayscale bytes directly from canvas canvas-pixels, matched
                directions via ray integration, and decoded them successfully.
              </span>
            </div>
          ) : (
            <div className="p-3 rounded-xl border border-dashed border-zinc-800 bg-zinc-900/10 text-center py-6 text-zinc-500 flex flex-col items-center gap-1.5 font-sans text-xs">
              ...
              <span>
                No scan run yet. Write some text or a pangram, then click "Run Interactive Scan" or
                "Instant Decode" to read backing pixels.
              </span>
            </div>
          )}
        </div>
                ...
                className={`border-l-2 pl-2 ${
                  log.includes("SUCCESS")
                    ...
                    : log.includes("ERROR") || log.includes("FAIL")
                      ? "border-red-500 text-red-400 bg-red-500/5 py-1"
                      : log.startsWith("Glyph") || log.startsWith("[Cell")
