...
export const UniversalAppSimulator: React.FC<SimulatorProps> = ({ appId, appName, initialUrl }) => {
  ...
  const [urlInput, setUrlInput] = useState(
    initialUrl ||
      `https://ais-dev-bpnlavv7oycjvturfkgjwa-990435940105.europe-west2.run.app?app=${appId}`,
  );
  ...
  const [vaultLogs, setVaultLogs] = useState<string[]>([
    ...
    "[VAULT] Connection status: secure",
  ]);
  ...
  const [visionAnalysis, setVisionAnalysis] = useState(
    "Select an image to activate Agentic Vision scanning loop...",
  );
  ...
  const [resolverTranscript, setResolverTranscript] = useState(
    "User: Bill says payment failed. Bill logs say account active. Database logs say card expired.",
  );
  const [resolvedRecords, setResolvedRecords] = useState(
    "Click resolve to merge conflicting customer records.",
  );
  ...
  const [expenses, setExpenses] = useState([
    { desc: "Stripe API Webhook pipeline", amount: 45.0, paidBy: "White Rabbit" },
    { desc: "Ollama local RAM host nodes", amount: 120.0, paidBy: "System Node" },
  ]);
  ...
  const handleSendTelegram = () => {
    ...
    setTelegramMessages((prev) => [
      ...prev,
      { sender: "You", text: telegramInput, time: "Just now" },
    ]);
  ...
  const handleKitchenAction = () => {
    ...
    setTimeout(
      () => setKitchenSteps((prev) => [...prev, "[KITCHEN] Mounting smart toaster. Code: 0x992"]),
      500,
    );
    setTimeout(
      () => setKitchenSteps((prev) => [...prev, "[KITCHEN] Calibrating stove heater to 180C"]),
      1000,
    );
    setTimeout(
      () =>
        setKitchenSteps((prev) => [
          ...prev,
          "[KITCHEN] Tool Call: bake_cake(duration_seconds=300)",
        ]),
      1500,
    );
    setTimeout(
      () =>
        setKitchenSteps((prev) => [
          ...prev,
          "[KITCHEN] SUCCESS: Agent finished recipe loop safely.",
        ]),
      2000,
    );
  };
          ...
          <div className="h-full w-full overflow-y-auto bg-zinc-950 p-6 flex flex-col justify-start">
              ...
              <div className="max-w-4xl mx-auto w-full flex flex-col h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
                <div className="bg-zinc-950 p-4 border-b border-zinc-800 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8.5 h-8.5 bg-blue-600 rounded-full flex items-center justify-center font-bold text-xs text-white">
                      WR
                    </div>
                    <div>
                ...
                <div className="flex-1 p-4 space-y-3 overflow-y-auto font-sans text-xs">
                  {telegramMessages.map((msg, i) => (
                    ...
                    >
                      <div
                        className={`p-3 rounded-2xl max-w-md ${msg.sender === "You" ? "bg-indigo-600 text-white" : "bg-zinc-800 text-zinc-300"}`}
                      >
                        <div className="font-bold text-[9px] text-zinc-400 mb-0.5">
                          {msg.sender}
                        </div>
                        <div>{msg.text}</div>
            ...
            {appId.includes("build_vault") && (
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-5 h-[500px]">
                <div className="md:col-span-7 bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col justify-between overflow-hidden">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                      ...
                      <span className="text-[9px] font-mono bg-zinc-950 border border-zinc-800 px-2 py-0.5 text-amber-300 rounded">
                        2 Active Builds
                      </span>
                    </div>
                    ...
                    <div className="space-y-2.5">
                      {builds.map((b) => (
                        <div
                          key={b.id}
                          className="bg-zinc-950 p-3.5 rounded-xl border border-zinc-850 flex items-center justify-between gap-3 text-xs"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-zinc-500 font-mono mt-0.5">
                                Size: {b.size} • Compiled safely
                              </div>
                            </div>
                          </div>
                          <span
                            className={`text-[9px] uppercase font-mono px-2 py-0.5 rounded ${b.status === "deployed" ? "bg-emerald-500/15 text-emerald-300 border border-emerald-500/30" : "bg-blue-500/15 text-blue-300 border border-blue-500/30"}`}
                          >
                            {b.status}
                          </span>
                        </div>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setBuilds((prev) => [
                        {
                          id,
                          name: `v3.0.0-draft-${prev.length + 1}`,
                          size: "4.8 MB",
                          status: "compiled",
                          date: "Just now",
                        },
                      ...
                      setVaultLogs((prev) => [
                        ...prev,
                        `[VAULT] Staging directory bundle compiled successfully: v3.0.0-draft`,
                      ]);
                    }}
                ...
                <div className="md:col-span-5 bg-zinc-950 border border-zinc-850 rounded-2xl p-4 flex flex-col overflow-hidden">
                  <span className="text-[10px] font-bold font-mono text-zinc-500 uppercase tracking-wider mb-2">
                    Vault Audit Logs
                  </span>
              ...
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                  ...
                  >
                    ...
                    <span>
                      {isCompilingJson ? "Compiling Schema..." : "Synthesize Detailed JSON"}
                    </span>
                  </button>
                </div>
            ...
            {appId.includes("agentic-vision") && (
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                <div className="md:col-span-7 bg-zinc-950 border border-zinc-850 rounded-2xl p-5 flex flex-col h-[340px] justify-between">
                  <span className="text-[10px] font-bold font-mono text-zinc-500 uppercase tracking-wider block">
                    Annotated Scan Report Viewport
                  </span>
              ...
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                  ...
                  >
                    ...
                    <span>
                      {isGeneratingUi ? "Evolving UI Components..." : "Compile UI Component"}
                    </span>
                  </button>
                </div>
            ...
            {appId.includes("data-resolver") && (
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-mono uppercase text-zinc-500">
                      Unstructured Conflict Transcripts
                    </span>
                    <textarea
                ...
                <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-5 flex flex-col justify-between h-[340px]">
                  <span className="text-[10px] font-bold font-mono text-zinc-500 uppercase tracking-wider block">
                    Resolved Clean Database Entry JSON
                  </span>
            ...
            {appId.includes("function-call-kitchen") && (
              <div className="max-w-4xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                  ...
                  <span className="text-[9px] font-mono uppercase bg-zinc-950 border border-zinc-800 px-2 py-0.5 text-amber-400 rounded">
                    100 simulated appliances
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                    <div className="w-10 h-10 bg-amber-500/10 rounded-lg flex items-center justify-center text-amber-500 text-lg font-bold group-hover:scale-105 transition-transform">
                      🍞
                    </div>
                    <div className="font-bold text-xs text-white">Bake Bread Agentic Pipeline</div>
                    <p className="text-[10px] text-zinc-500">
                      Initiates toast oven parameters and triggers bake_toast() function loop.
                    </p>
                  </button>
                  ...
                  <button
                    ...
                    className="p-4 bg-zinc-950 border border-zinc-850 hover:border-zinc-700 rounded-xl text-left space-y-2 group transition-all"
                  >
                    <div className="w-10 h-10 bg-purple-500/10 rounded-lg flex items-center justify-center text-purple-500 text-lg font-bold group-hover:scale-105 transition-transform">
                      🍳
                    </div>
                    <div className="font-bold text-xs text-white">Stove Temp Controller</div>
                    <p className="text-[10px] text-zinc-500">
                      Auto calibrates stove induction elements to safe temperatures.
                    </p>
                  </button>
                  ...
                  <button
                    ...
                    className="p-4 bg-zinc-950 border border-zinc-850 hover:border-zinc-700 rounded-xl text-left space-y-2 group transition-all"
                  >
                    <div className="w-10 h-10 bg-cyan-500/10 rounded-lg flex items-center justify-center text-cyan-500 text-lg font-bold group-hover:scale-105 transition-transform">
                      🍰
                    </div>
                    <div className="font-bold text-xs text-white">Simulated Kitchen Assistant</div>
                    <p className="text-[10px] text-zinc-500">
                      Parallel triggers 100 tools via multi-model Gemini function calling.
                    </p>
                  </button>
                </div>
              ...
              <div className="max-w-4xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (task) {
                        setChores((prev) => [
                          ...prev,
                          { task, assign: "System Node", done: false },
                        ]);
                      }
                    }}
                ...
                <div className="space-y-2 text-xs">
                  {chores.map((chore, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-zinc-950 rounded-xl border border-zinc-850 flex items-center justify-between gap-3 group"
                    >
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() =>
                        ...
                        <span
                          className={`font-semibold ${chore.done ? "line-through text-zinc-500" : "text-zinc-200"}`}
                        >
                          {chore.task}
                        </span>
                      </div>
                      <span className="text-[10px] font-mono px-2 py-0.5 bg-zinc-900 border border-zinc-800 rounded text-zinc-400 font-bold uppercase">
                        {chore.assign}
                      </span>
                    </div>
                  ))}
              ...
              <div className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-5">
                <div className="md:col-span-7 bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="space-y-2 text-xs max-h-[180px] overflow-auto">
                    {expenses.map((e, idx) => (
                      <div
                        key={idx}
                        className="p-3 bg-zinc-950 rounded-xl border border-zinc-850 flex items-center justify-between text-zinc-300 font-mono"
                      >
                  ...
                  <button
                    onClick={() => {
                      ...
                      setExpenses((prev) => [
                        ...prev,
                        { desc: newExpenseDesc, amount: parseFloat(newExpenseAmt), paidBy: "You" },
                      ]);
                      setNewExpenseDesc("");
                      setNewExpenseAmt("");
                    }}
                ...
                <div className="md:col-span-5 bg-zinc-950 border border-zinc-850 rounded-2xl p-5 flex flex-col justify-center items-center text-center">
                  ...
                  <p className="text-[10px] text-zinc-500 mt-2 font-mono">
                    Calculated safely across database ledger nodes.
                  </p>
                </div>
              </div>
            )}
            ...
            {appId.includes("sky-metropolis") && (
              <div className="max-w-4xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                  ...
                  <span className="text-[10px] font-mono bg-zinc-950 px-2 py-0.5 rounded text-emerald-400">
                    MAYOR MODE STATUS: STABLE
                  </span>
                </div>
                ...
                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-850 space-y-2 text-xs">
                  <span className="text-[10px] font-mono text-indigo-400 uppercase font-bold">
                    Active Directive from Gemini:
                  </span>
                  <p className="text-zinc-300 italic leading-relaxed">
                    "Optimize visual viewport layout in District 5, configure central solar grids to
                    handle high-performance mathematical iterations."
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="max-w-4xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
                ...
                <div className="grid grid-cols-8 gap-2">
                  {Array.from({ length: 32 }).map((_, i) => (
                    <div
                      key={i}
                      className={`h-12 rounded bg-zinc-800 border border-zinc-700 ${i % 3 === 0 ? "bg-rose-500/20 border-rose-500/50" : ""}`}
                    ></div>
                  ))}
                </div>
              </div>
              ...
              <div className="max-w-4xl mx-auto w-full grid grid-cols-3 gap-5">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="space-y-2 text-xs text-zinc-400">
                    <p>
                      <span className="text-red-400">0:00</span> - Introduction to tensors...
                    </p>
                    <p>
                      <span className="text-red-400">1:15</span> - Matrix multiplication rules...
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-[#c0c0c0] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-[#808080] p-1 shadow-lg max-w-lg mx-auto">
                  <div className="bg-[#000080] text-white font-bold px-2 py-1 flex justify-between items-center text-sm">
                    ...
                    <div className="flex gap-1">
                      <div className="w-4 h-4 bg-[#c0c0c0] text-black text-xs text-center border-t border-l border-white border-b border-r border-[#808080]">
                        X
                      </div>
                    </div>
                  </div>
                  <div className="p-4 space-y-4">
                    ...
                    <input
                      type="text"
                      className="w-full bg-white border-t-2 border-l-2 border-[#808080] border-b-2 border-r-2 border-white px-2 py-1 text-black text-sm"
                      defaultValue="Create a retro styled OS window..."
                    />
                    <button className="bg-[#c0c0c0] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-[#808080] px-4 py-1 text-black text-sm active:border-t-[#808080] active:border-l-[#808080] active:border-b-white active:border-r-white">
                      Generate
                    </button>
                  </div>
                </div>
            ...
            {appId.includes("echoscript") && (
              <div className="max-w-4xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 space-y-8 flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-zinc-400 text-sm font-mono text-center">
                  Listening for audio stream... <br /> [Transcribing to text]
                </p>
              </div>
            )}
            ...
            {appId.includes("chat-with-docs") && (
              <div className="max-w-5xl mx-auto w-full grid grid-cols-2 gap-0 h-[500px] border border-zinc-800 rounded-2xl overflow-hidden">
                <div className="bg-zinc-950 p-6 flex flex-col items-center justify-center border-r border-zinc-800 relative">
                  ...
                  <div className="absolute top-4 right-4 bg-blue-600 text-white text-[10px] px-2 py-1 rounded font-bold uppercase">
                    PDF Loaded
                  </div>
                </div>
                <div className="bg-zinc-900 p-6 flex flex-col justify-between">
                  <div className="space-y-4 flex flex-col">
                    <div className="bg-zinc-800 p-3 rounded-lg text-sm text-zinc-200 self-start w-3/4">
                      What does this document say about vector embeddings?
                    </div>
                    <div className="bg-blue-600/20 border border-blue-500/30 p-3 rounded-lg text-sm text-blue-200 self-end w-3/4">
                      According to page 4, vector embeddings are generated using a 768-dimensional
                      space...
                    </div>
                  </div>
                  <div className="mt-4 flex gap-2">
                    <input
                      type="text"
                      className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-sm text-white focus:outline-none"
              ...
              <div className="max-w-4xl mx-auto w-full h-[400px] bg-zinc-900 border border-zinc-800 rounded-2xl p-8 flex flex-col items-center justify-center space-y-8">
                ...
                <div className="text-center">
                  ...
                  <p className="text-zinc-500 text-sm mt-2">
                    Breeding generation 402... Fitness score: 0.94
                  </p>
                </div>
              </div>
            )}
            ...
            {appId.includes("action-replay") && (
              <div className="max-w-4xl mx-auto w-full h-[400px] bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col">
                ...
                <div className="h-20 bg-zinc-950 border-t border-zinc-800 flex items-center justify-center gap-6">
                  ...
                  <button className="w-12 h-12 rounded-full bg-violet-600 flex items-center justify-center text-white">
              ...
              <div className="max-w-4xl mx-auto w-full h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl flex overflow-hidden">
                <div className="w-64 bg-zinc-950 border-r border-zinc-800 p-4 space-y-4">
                  ...
                  {[
                    ...
                    <div
                      key={i}
                      className="text-sm text-zinc-400 hover:text-white cursor-pointer py-1"
                    >
                      {item}
                    </div>
                  ))}
                ...
                <div className="flex-1 p-8 space-y-6">
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-12 pr-4 py-4 text-white focus:outline-none focus:border-teal-500"
                      placeholder="Ask the manual anything..."
                    />
                  </div>
                  <div className="bg-teal-500/10 border border-teal-500/20 rounded-xl p-6">
                    ...
                    <p className="text-zinc-300 text-sm leading-relaxed">
                      The manual is powered by semantic search. You don't need to use exact
                      keywords. Just describe the problem you're trying to solve.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="max-w-4xl mx-auto w-full h-[400px] bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden relative">
                ...
                <div className="absolute bottom-6 right-6 bg-zinc-950/80 backdrop-blur border border-zinc-800 p-4 rounded-xl flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-xs font-mono">
                    ...
                    <span className="text-zinc-300">45.21</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-mono">
                    <span className="text-green-400 font-bold">Y:</span>{" "}
                    <span className="text-zinc-300">12.08</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-mono">
                    <span className="text-blue-400 font-bold">Z:</span>{" "}
                    <span className="text-zinc-300">-8.44</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {appId.includes("drive-explorer") && (
              <div className="max-w-4xl mx-auto w-full h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex">
                <div className="w-64 bg-zinc-950 border-r border-zinc-800 p-4 space-y-4">
                  <div className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-4">
                    Google Drive
                  </div>
                  <div className="space-y-2">
                    {["My Drive", "Shared with me", "Recent", "Starred", "Trash"].map((item, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 text-sm text-zinc-400 hover:text-white cursor-pointer px-2 py-1.5 rounded-lg hover:bg-zinc-800/50"
                      >
                        <FolderOpen size={16} />
                        {item}
                      </div>
                ...
                <div className="flex-1 p-6">
                  <div className="flex items-center gap-4 mb-8">
                    <div className="flex-1 relative">
                      ...
                      <input
                        type="text"
                        className="w-full bg-zinc-950 border border-zinc-800 rounded-full pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    {Array.from({ length: 8 }).map((_, i) => (
                      <div
                        key={i}
                        className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex flex-col items-center justify-center gap-3 hover:bg-zinc-800/50 cursor-pointer"
                      >
                        <div className="w-12 h-12 bg-blue-500/10 rounded-lg flex items-center justify-center text-blue-500">
                          <FileText size={24} />
                        </div>
                        <span className="text-xs text-zinc-300 text-center line-clamp-2">
              ...
              <div className="max-w-4xl mx-auto w-full h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col">
                <div className="flex items-center gap-4 mb-8 pb-4 border-b border-zinc-800">
                  ...
                  <div className="ml-auto flex gap-2">
                    <button className="bg-emerald-600 text-white px-4 py-1.5 rounded-md text-sm font-semibold">
                      Deploy Squad
                    </button>
                  </div>
                </div>
                <div className="flex-1 grid grid-cols-3 gap-6">
                  {["Researcher (A1)", "Coder (A2)", "Reviewer (A3)"].map((role, i) => (
                    <div
                      key={i}
                      className="bg-zinc-950 border border-zinc-800 rounded-xl p-5 flex flex-col items-center text-center space-y-4 relative overflow-hidden"
                    >
            ...
            {appId.includes("ai-radio") && (
              <div className="max-w-md mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl flex flex-col items-center">
                <h2 className="text-xl font-black text-orange-500 tracking-widest mb-6">
                  DIY RADIO
                </h2>
                <div className="w-48 h-48 rounded-full border-[8px] border-zinc-800 bg-zinc-950 flex items-center justify-center relative shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]">
                  ...
                  <div
                    className="absolute w-full h-1 bg-orange-500/50 origin-left animate-spin"
                ...
                <div className="w-full mt-8 space-y-4">
                  ...
                  <div className="flex justify-between items-center px-4">
                    ...
                    <button className="w-16 h-16 rounded-full bg-orange-600 flex items-center justify-center text-white shadow-lg shadow-orange-600/20">
            ...
            {appId.includes("iron-men-arcade") && (
              <div className="max-w-3xl mx-auto w-full bg-zinc-900 border-[8px] border-red-900/50 rounded-[2rem] p-6 shadow-[0_0_40px_rgba(220,38,38,0.2)]">
                <div className="bg-black border-4 border-zinc-800 rounded-xl h-[400px] relative overflow-hidden flex flex-col items-center justify-center">
                  <h1
                    className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-400 to-red-600 tracking-tighter italic mb-8"
                    style={{ textShadow: "0 4px 10px rgba(220,38,38,0.5)" }}
                  >
                    IRON MEN ARCADE
                  </h1>
                  <div className="text-white text-xl animate-pulse font-mono mb-12">
                    INSERT COIN
                  </div>
                  <div className="absolute bottom-4 text-xs font-mono text-zinc-600">
                    © 2026 CYBERNETIC ENTERTAINMENT
                  </div>
                </div>
              </div>
            )}
            ...
            {appId.includes("diy-comic") && (
              <div className="max-w-4xl mx-auto w-full h-[550px] bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex gap-6">
                <div className="w-48 space-y-4 shrink-0">
                  <div className="bg-violet-600 text-white font-bold text-center py-2 rounded-lg text-sm">
                    New Panel
                  </div>
                  <div className="space-y-2">
                    ...
                    {["Hero", "Villain", "Sidekick"].map((c) => (
                      <div
                        key={c}
                        className="bg-zinc-950 border border-zinc-800 p-2 rounded text-sm text-zinc-300 hover:border-violet-500 cursor-pointer"
                      >
                        {c}
                      </div>
                    ))}
                  ...
                  <div className="space-y-2 pt-4">
                    <div className="text-xs text-zinc-500 uppercase font-bold pl-1">
                      Speech Bubbles
                    </div>
                ...
                <div className="flex-1 bg-white rounded-lg border-4 border-black p-4 relative">
                  <div className="absolute top-4 left-4 bg-yellow-400 border-2 border-black px-3 py-1 font-bold italic transform -rotate-2">
                    MEANWHILE...
                  </div>
              ...
              <div className="max-w-3xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                  <div className="flex gap-2">
                    <input
                      type="text"
                      className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-cyan-500"
                      placeholder="Paste URL here..."
                      defaultValue="https://example.com/article"
                    />
                    <button className="bg-cyan-600 text-white px-6 rounded-lg font-bold">
                      Extract
                    </button>
                  </div>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-zinc-500 text-xs mt-2">
                          Converting semantics to raw ink representation
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="max-w-4xl mx-auto w-full h-[500px] bg-zinc-950 border border-zinc-800 rounded-2xl p-8 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center h-full">
                  <div className="flex w-full justify-between items-center mb-12">
                    <div className="text-red-500 font-mono text-2xl font-bold bg-red-950/50 px-4 py-2 border border-red-900 rounded">
                      04:59
                    </div>
                    <div className="flex gap-4">
                      ...
                      <div className="text-center">
                        <div className="text-xs text-blue-500 uppercase tracking-widest mb-1">
                          Blue Team
                        </div>
              ...
              <div className="max-w-5xl mx-auto w-full h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl flex overflow-hidden">
                <div className="w-1/3 bg-zinc-950 border-r border-zinc-800 p-6 flex flex-col">
                  <h2 className="text-white font-bold mb-4 flex items-center gap-2">
                    <Layers className="text-emerald-500" size={20} /> Artifact Logic
                  </h2>
                ...
                <div className="w-2/3 bg-zinc-100 flex items-center justify-center relative p-8">
                  <div className="absolute top-4 right-4 bg-black/10 px-3 py-1 rounded text-xs font-mono text-black/60 font-bold">
                    PREVIEW
                  </div>
                  <div className="w-full max-w-md space-y-6">
                    ...
                    <button className="bg-emerald-600 text-white w-full py-3 rounded-lg font-bold shadow-lg mt-4">
                      Interact
                    </button>
                  </div>
                </div>
                ...
                <div className="max-w-3xl mx-auto w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-5 text-center shadow-2xl animate-scaleIn">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-zinc-400 max-w-md mx-auto leading-relaxed">
                      This app node is running in standboxed OS host pipelines. You can use the
                      fully connected simulator below, or configure the actual Shared Deployment URL
                      in the address bar above to embed the live iframe.
                    </p>
                  </div>
                  ...
                  <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-850/60 max-w-md mx-auto space-y-4">
                    ...
                    <p className="text-[10px] text-zinc-600 font-mono">
                      ID: {appId} • Sandbox Compile Node 3000
                    </p>
                  </div>
                </div>
              )}
