...
export const CostAnalyticsApp: React.FC = () => {
  ...
  const avgCostPerK = useMemo(
    () => (totalCost > 0 ? ((totalCost / totalTokens) * 1000).toFixed(4) : "0"),
  ...
  const budgetUtilization =
    totalBudgetLimit > 0 ? ((totalBudgetSpent / totalBudgetLimit) * 100).toFixed(1) : "0";

  const freeTierUsage = costByProvider
    .filter((p) => parseFloat(p.costPerK) === 0)
    .reduce((sum, p) => sum + p.tokens, 0);
  const paidTierUsage = costByProvider
    .filter((p) => parseFloat(p.costPerK) > 0)
    .reduce((sum, p) => sum + p.tokens, 0);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">
                Track and optimize AI spending across providers
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {costByProvider.map((item, i) => (
                  <div
                    key={i}
                    className="bg-slate-700 rounded p-2 hover:bg-slate-600 cursor-pointer transition"
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400">
                      <span>
                        {item.tokens.toLocaleString()} tokens • {item.count} calls
                      </span>
                      <span
                        className={
                          parseFloat(item.costPerK) === 0 ? "text-emerald-400" : "text-orange-400"
                        }
                      >
                        ${item.costPerK}/1K
                      </span>
              ...
              <div className="space-y-2">
                {costByTask.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400">
                      <span>
                        {item.tokens.toLocaleString()} tokens • {item.count} tasks
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div className="mb-4 bg-slate-700 rounded p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-yellow-400 font-bold">
                    ${totalBudgetSpent.toFixed(2)} / ${totalBudgetLimit}
                  </span>
                </div>
                <div className="w-full bg-slate-600 rounded-full h-2 overflow-hidden">
                  <div
                    className={`h-full transition-all ${
                      parseFloat(budgetUtilization) < 50
                        ? "bg-emerald-500"
                ...
                <div className="text-xs text-slate-400 mt-1">{budgetUtilization}% utilized</div>
              </div>
            )}
                ...
                <div
                  ...
                  className={`rounded p-2 border ${
                    budget.monthlyLimit === 0
                      ...
                      : budget.spent / budget.monthlyLimit > 0.8
                  ...
                  {budget.monthlyLimit === 0 ? (
                    ...
                    <>
                      <div className="text-xs text-yellow-400 font-bold mt-1">
                        ${budget.spent.toFixed(2)} / ${budget.monthlyLimit}
                      </div>
          ...
          <div className="bg-emerald-950/30 border border-emerald-700/30 rounded-lg p-4">
            ...
            <ul className="space-y-2 text-sm text-emerald-300">
              <li>
                ✓ Great job! {freePercentage}% of tokens using free tier (Groq, Ollama, Gemini)
              </li>
              <li>• Consider routing DeepSeek to Groq for cost savings when possible</li>
              <li>
                • You have {paidTierUsage.toLocaleString()} paid tokens - review for optimization
              </li>
