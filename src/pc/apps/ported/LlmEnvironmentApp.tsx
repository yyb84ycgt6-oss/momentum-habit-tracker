...
const renderHeader = (title: string, onBack?: () => void) => (
  <div className="flex items-center justify-between p-4 border-b border-zinc-800">
    {onBack ? (
      <button
        onClick={onBack}
        className="p-2 -ml-2 rounded-full bg-zinc-900 text-zinc-300 hover:text-white hover:bg-zinc-800 transition-colors"
      >
        <ChevronLeft size={20} />
      </button>
    ) : (
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="bg-zinc-900 border border-emerald-900/50 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-emerald-400 text-sm uppercase tracking-wider font-semibold">
              Active Metrics
            </h3>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-blue-400 text-sm uppercase tracking-wider font-semibold">
              Health Connectivity
            </h3>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-purple-400 text-sm uppercase tracking-wider font-semibold">
              Model Routing
            </h3>
          </div>
          <div className="text-sm text-zinc-400">
            Current fallback chain is fully operational. Overruling local constraints if load
            exceeds 95%.
          </div>
        </div>
      </div>
...
export const LlmEnvironmentApp: React.FC = () => {
  ...
  const selectModel = (name: string) => {
    setActiveModel(name);
    try {
      localStorage.setItem("llm_active_model", name);
    } catch {}
  ...
  const saveCustom = () => {
    try {
      localStorage.setItem("llm_custom_backend", JSON.stringify(custom));
    } catch {}
  ...
  const deleteCustom = () => {
    ...
    try {
      localStorage.removeItem("llm_custom_backend");
    } catch {}
    setCustom({
      name: "Custom Backend",
      baseUrl: "",
    ...
    <div className="flex flex-col h-full bg-zinc-950 text-white">
      <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-center">
        ...
        <div className="w-full max-w-sm space-y-4">
          <button
            ...
            className="w-full flex items-start gap-4 p-4 rounded-2xl hover:bg-zinc-900/50 transition-colors text-left"
          >
            <Smartphone size={24} className="text-zinc-400 mt-1" />
            <div>
              <h3 className="font-semibold text-zinc-200">Local Models</h3>
              <p className="text-sm text-zinc-500">
                Run models privately on your device. Works best on newer hardware.
              </p>
            </div>
          </button>
          <button
            onClick={() => setCurrentView("openrouter")}
            className="w-full flex items-start gap-4 p-4 rounded-2xl hover:bg-zinc-900/50 transition-colors text-left"
          >
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500">
                Access a wide range of hosted models including OpenAI, Anthropic, and more.
              </p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500">
                Connect your own backend or API to use custom models.
              </p>
            </div>
          </button>
          <button
            onClick={() => setCurrentView("diagnostics")}
            className="w-full flex items-start gap-4 p-4 rounded-2xl hover:bg-zinc-900/50 transition-colors text-left border border-zinc-800 bg-zinc-900/20"
          >
            <Activity size={24} className="text-emerald-400 mt-1" />
            <div>
              <h3 className="font-semibold text-emerald-400">System Diagnostics</h3>
              <p className="text-sm text-zinc-500">
                Monitor active model token usage, response latency, and connectivity metrics.
              </p>
            </div>
          </button>
        </div>
      ...
      <div className="p-6 border-t border-zinc-900">
        <button
          ...
          className="w-full bg-zinc-100 hover:bg-white text-black font-semibold rounded-full py-4 transition-colors"
        >
          Choose a model
        </button>
      </div>
    ...
    <div className="flex flex-col h-full bg-zinc-950 text-white relative">
      ...
      <div className="px-6 py-4">
        ...
        <div className="bg-zinc-900/80 rounded-3xl overflow-hidden border border-zinc-800">
          <button
            onClick={() => setCurrentView("openrouter")}
            className="w-full flex items-center justify-between p-5 border-b border-zinc-800/80 hover:bg-zinc-800/50 transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-zinc-900/80 rounded-3xl overflow-hidden border border-zinc-800">
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 border-b border-zinc-800/80 last:border-0 transition-colors ${activeModel === model.name ? "bg-emerald-500/10" : "hover:bg-zinc-800/50"}`}
                >
                  <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="bg-zinc-900/80 rounded-3xl overflow-hidden border border-zinc-800">
              <button
                onClick={() => selectModel("LFM2-350M-ENJP-MT")}
                className={`w-full flex items-center justify-between p-4 transition-colors ${activeModel === "LFM2-350M-ENJP-MT" ? "bg-emerald-500/10" : "hover:bg-zinc-800/50"}`}
              >
                <div className="flex items-center gap-4">
                  <div className="w-8 h-8 rounded bg-white flex items-center justify-center text-black">
                    <span className="font-bold">L</span>
  ...
  const renderOpenRouter = () => (
    <div className="flex flex-col h-full bg-zinc-950 text-white">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <p className="text-sm text-zinc-400 text-center mb-8 px-4">
          OpenRouter provides routing to various LLM providers. Set up autopay to avoid
          interruptions.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <button
              onClick={() =>
                setNotice("Add an API key or Base URL above, then reconnect to load models.")
              }
  ...
  const renderCustom = () => (
    <div className="flex flex-col h-full bg-zinc-950 text-white">
      <div className="flex items-center justify-between p-4 border-b border-zinc-800">
        ...
        <button
          onClick={saveCustom}
          className="px-4 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-white font-medium rounded-full border border-zinc-700 text-sm transition-colors"
        >
          Save
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <div className="space-y-8">
          <div>
            <h3 className="text-sm font-semibold text-zinc-500 mb-2 px-2">
            ...
            <div className="bg-zinc-900/80 rounded-3xl overflow-hidden border border-zinc-800">
              ...
              <div className="flex items-center p-4 border-b border-zinc-800/80">
                ...
                <input
                  ...
                  placeholder="http://192.168.1.100:123"
              ...
              <div className="flex items-center p-4 border-b border-zinc-800/80">
                ...
                <input
                  ...
                  onChange={(e) => setCustomField("chatPath", e.target.value)}
                  className="flex-1 bg-transparent border-none outline-none text-white text-[15px]"
                />
              </div>
              <div className="flex items-center p-4 border-b border-zinc-800/80">
                ...
                <input
                  type="text"
                  value={custom.modelsPath}
                  onChange={(e) => setCustomField("modelsPath", e.target.value)}
                  className="flex-1 bg-transparent border-none outline-none text-white text-[15px]"
                />
              </div>
              <div className="flex items-center p-4">
                ...
                <input
                  type="text"
                  value={custom.auth}
                  onChange={(e) => setCustomField("auth", e.target.value)}
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 px-2">
              Specify your model identifier, or reload your custom models.
            </p>
          </div>
          ...
          <div>
            ...
            <button
              ...
              className="w-full bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 text-left font-medium text-red-500 hover:bg-zinc-800/50 transition-colors"
            >
              Delete
            </button>
          </div>
      ...
      <div className="p-4 border-t border-zinc-900 flex justify-center">
        <button
          onClick={() => setCurrentView("providers")}
          className="text-zinc-500 hover:text-white transition-colors text-sm underline"
        >
          Cancel
        </button>
      </div>
