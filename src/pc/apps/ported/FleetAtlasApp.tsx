...
const FleetAtlasApp: React.FC = () => {
  ...
  const specs = useMemo(() => [...new Set(nodes.map((n) => n.spec))].sort(), [nodes]);

  const edgeCount = useMemo(() => {
    let c = 0;
    for (let i = 0; i < nodes.length; i++)
      for (let j = i + 1; j < nodes.length; j++) if (nodes[i].spec === nodes[j].spec) c++;
    return c;
  }, [nodes]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (const p of order) {
        const size =
          (p.n.status === "working" ? 6 : p.n.status === "fixed" ? 5.2 : 4) * p.scale * dpr;
  ...
  return (
    <div className="w-full h-full relative bg-[#05070d] overflow-hidden select-none text-zinc-200">
      ...
      <div className="absolute top-4 left-4 pointer-events-none">
        <div className="text-[17px] font-bold tracking-[2px] text-zinc-100">KNOWLEDGE NETWORK</div>
        ...
        {specs.map((spec) => (
          <button
            ...
            style={{
              ...
              background: filterSpec === spec ? SPEC_COLORS[spec] + "22" : "transparent",
      ...
      {selected && (
        ...
        >
          ...
          <div className="text-sm font-semibold text-zinc-100 mb-1">{selected.id}</div>
          <div className="text-[11px] text-zinc-500 mb-2">{selected.spec} condenser</div>
          <span
