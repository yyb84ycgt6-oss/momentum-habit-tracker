    ...
    const gameLoop = (time: number) => {
      ...
      if (time - s.lastSpawn > Math.max(400, 1200 - s.score * 40)) {
        s.enemies.push({
          ...
          w: 40,
          h: 40,
          speed: 3 + Math.random() * 3 + s.score * 0.15,
          id: s.idCounter++,
        });
        s.lastSpawn = time;
      }
      ...
      for (let i = s.enemies.length - 1; i >= 0; i--) {
        ...
        e.y += e.speed;
        if (
          e.y + e.h > height - s.playerH - 10 &&
          e.x < s.playerX + s.playerW &&
          e.x + e.w > s.playerX
        ) {
          setGameOver(true);
          setIsPlaying(false);
  ...
  return (
    <div className="h-full w-full bg-zinc-950 flex flex-col relative" ref={containerRef}>
      ...
      <canvas
        ...
        style={{ backgroundImage: "linear-gradient(to bottom, #1e1b4b 0%, #000000 100%)" }}
      />
      {(!isPlaying || gameOver) && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md">
          {gameOver && (
            <div className="mb-8 text-center animate-in fade-in zoom-in duration-300">
              <h2 className="text-5xl font-black text-red-500 mb-2 tracking-wider">
                SYSTEM FAILURE
              </h2>
              <p className="text-xl text-yellow-400 font-mono">
                FINAL SCORE: {state.current.score}
              </p>
            </div>
          )}
          <button
            onClick={initGame}
            className="group px-8 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-full transition-all shadow-[0_0_20px_rgba(220,38,38,0.6)] flex items-center gap-3"
          >
