        ...
        // Sequences (e.g. {WMEZ3E})
          ...
          if (endIdx === -1) {
            ...
            const chars = seqContent.split("").map((char) => {
              const direction =
                CHAR_TO_DIRECTION[char.toLowerCase()] ?? Math.floor(Math.random() * 8);
              return {
                char,
                direction,
          ...
          result.push({
            type: "sequence",
            raw: str[i],
            chars: [{ char: str[i], direction: Math.floor(Math.random() * 8), label: "Fallback" }],
          });
          i++;
        }
      }
    ...
    <div className="h-full w-full bg-[#050608] text-[#a9b2c3] font-mono flex flex-col overflow-hidden select-none">
      ...
      <div className="h-12 border-b border-[#141822] bg-[#090b11] px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase text-[#e2e8f0] tracking-widest">
            qpdb // Quantum Pod-State Matrix
          </span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[40%] min-w-[280px] border-r border-[#141822] bg-[#080a0f] flex flex-col overflow-y-auto custom-scrollbar p-5 space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-[10px] text-[#e2e8f0] font-black uppercase tracking-wider flex items-center gap-1.5">
                <Code size={12} className="text-amber-500" /> State Stream Code
              </span>
              <span className="text-[10px] text-[#52607a] font-mono">
          ...
          {/* Presets List */}
          <div className="space-y-2">
            <span className="text-[10px] text-[#8e9bb4] font-black uppercase tracking-wider">
              State Presets
            </span>
              ...
              {PRESETS.map((p, idx) => (
                ...
                >
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs font-bold text-[#e2e8f0] group-hover:text-amber-400 transition-colors">
                      {p.name}
                    </span>
                    <ArrowRight
                      size={10}
                      className="text-[#3b4861] group-hover:translate-x-1 transition-transform"
                    />
                  </div>
          ...
          <div className="p-4 bg-[#0a0d14] border border-[#141822] rounded-lg space-y-3.5">
            <div className="flex items-center gap-2 border-b border-[#141822] pb-2">
              ...
              <span className="text-[11px] font-black uppercase text-[#e2e8f0] tracking-wider">
                Atom Compass Orientation (Level 2)
              </span>
            </div>
            <p className="text-[10px] text-[#52607a] leading-relaxed">
              A single active atom <span className="text-amber-400">&lt;0&gt;</span> paired with its
              orientation stem represents 8 discrete angle states (1 bit + 3 bits of orientation
              metadata). Click any angle on the compass to highlight matching nodes in the parsed
              sequence.
            </p>
              ...
              <div className="space-y-1.5 text-[10px]">
                ...
                {activeCompassDir !== null ? (
                  <div className="space-y-1 text-amber-400">
                    ...
                    <div className="text-[9px] text-[#52607a]">
                      Angle: {DIRECTION_TO_LABEL[activeCompassDir].angle}°
                    </div>
                    <div className="text-[9px] text-[#52607a] max-w-[12rem] leading-snug">
                      Characters mapping to this angle:{" "}
                      {Object.entries(CHAR_TO_DIRECTION)
                  ...
                  <div className="text-[#3b4861] italic">
                    Select compass state to analyze vectors...
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-4 bg-[#0a0d14]/60 border border-[#141822] rounded-lg space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-[10px] text-[#e2e8f0] font-black uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles size={12} className="text-amber-500" /> Compile Intent with AI
              </span>
              <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded px-1 text-[8px] font-bold uppercase tracking-widest">
                GEMINI CORE
              </span>
            </div>
            <p className="text-[10px] text-[#52607a] leading-relaxed">
              Provide high-level configurations or actions, and let the AI compile them into a
              multi-layered stacked pod string.
            </p>
            <div className="flex gap-2">
              <input
                type="text"
          ...
          <div className="space-y-3 pt-2 border-t border-[#141822]">
            <span className="text-[11px] text-[#e2e8f0] font-black uppercase tracking-wider flex items-center gap-1.5">
              <Info size={13} className="text-amber-500" /> qpdb Paradigm Definition
            </span>
            ...
            <div className="grid grid-cols-1 gap-3 text-[10px] text-[#52607a] leading-relaxed">
              <div className="p-3 bg-[#0a0d14]/30 rounded-lg border border-[#141822]">
                ...
                <p>
                  The atomic unit of information (1 bit), modeled as a circle state. Active{" "}
                  <span className="text-amber-400">&lt;0&gt;</span> contains presence, whereas empty{" "}
                  <span className="text-zinc-500">&lt;&gt;</span> outlines physical storage capacity
                  with no state loaded.
                </p>
              </div>
              <div className="p-3 bg-[#0a0d14]/30 rounded-lg border border-[#141822]">
                <div className="text-[#8e9bb4] font-black uppercase mb-1">
                  2. Position & Stem Orientation
                </div>
                <p>
                  Extending the atom, the stem projects in one of 8 directions around the cell
                  center, representing 8 states (orientation value). Highly dense data
                  representation utilizing physical coordinate geometry.
                </p>
              </div>
              <div className="p-3 bg-[#0a0d14]/30 rounded-lg border border-[#141822]">
                <div className="text-[#8e9bb4] font-black uppercase mb-1">
                  3. Sequence Chain ($8^n$)
                </div>
                <p>
                  Chaining the active elements sequentially. Stacking nodes next to each other
                  creates exponential state expansion (<span className="text-amber-400">8ⁿ</span>{" "}
                  possibilities). Extrapolates deep data pathways with few tokens.
                </p>
              </div>
              <div className="p-3 bg-[#0a0d14]/30 rounded-lg border border-[#141822]">
                <div className="text-[#8e9bb4] font-black uppercase mb-1">
                ...
                <p>
                  Stacking sequences vertically or horizontally in 2D coordinate blocks (separated
                  by dashes/tildes). Creates fully structured functional Pod models for robust local
                  indexing.
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 bg-[#06080d] flex flex-col overflow-hidden relative">
          ...
          <div className="h-10 border-b border-[#141822] px-4 flex items-center justify-between bg-[#080b10] shrink-0">
            <span className="text-[10px] text-[#8e9bb4] font-black uppercase tracking-wider flex items-center gap-1.5">
              <Layers size={12} className="text-amber-500" /> Active Visual Matrix
            </span>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Play size={10} className={isSimulating ? "animate-pulse" : ""} />{" "}
                {isSimulating ? "Transmitting..." : "Transmit Pod State"}
              </button>
            </div>
          </div>
                        ...
                        >
                          <div className="flex items-center gap-1 border-b border-[#141822] pb-1.5 mb-3 w-full justify-between">
                            <span className="text-[9px] font-black text-[#52607a] uppercase tracking-wider flex items-center gap-1.5">
                              <Network size={12} className="text-amber-500" /> Sequence Chain (
                              {el.chars.length} Stems)
                            </span>
                            <button
                              onClick={() => {
                                setSelectedElementIndex(isElementSelected ? null : elIdx);
                          ...
                          <div className="flex flex-wrap items-center justify-center gap-6 py-2">
                            {el.chars.map((node, nodeIdx) => {
                              const isNodeSelected =
                                selectedNodeIndex?.elementIdx === elIdx &&
                                selectedNodeIndex?.nodeIdx === nodeIdx;
                              const isMatchedByCompass =
                                activeCompassDir !== null && node.direction === activeCompassDir;
                              ...
                              return (
                                <div
                                  ...
                                  onClick={(e) => {
                                    ...
                                    setSelectedNodeIndex(
                                      isNodeSelected ? null : { elementIdx: elIdx, nodeIdx },
                                    );
                                    setSelectedElementIndex(null);
                                  }}
                                ...
                                >
                                  ...
                                  <span
                                    className={`text-[10px] font-bold font-mono mt-2 px-1.5 py-0.5 rounded ${
                                      isNodeSelected
          ...
          <div className="h-44 border-t border-[#141822] bg-[#07090d] p-4 flex flex-col justify-between shrink-0 relative">
            ...
            {selectedNodeIndex !== null ? (
                  ...
                  <div className="flex-1 flex gap-6 items-start">
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-4 text-xs">
                      <div className="space-y-1">
                        <span className="text-[9px] text-[#52607a] uppercase font-bold">
                          Element Class
                        </span>
                        <div className="text-[#e2e8f0] font-bold flex items-center gap-1.5">
                          <Network size={12} className="text-amber-400" /> Sequence Node
                        </div>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[9px] text-[#52607a] uppercase font-bold">
                        ...
                        <div className="text-[#63e2b7] font-bold">
                          {DIRECTION_TO_LABEL[node.direction].arrow} {node.label} (State{" "}
                          {node.direction})
                        </div>
                      </div>
                      <div className="space-y-1">
                        ...
                        <div className="text-amber-400 font-mono font-bold">
                          {node.direction.toString(2).padStart(3, "0")} (3 bits)
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      {el.type === "sequence" && decodedBytes[selectedElementIndex] && (
                        <div className="space-y-1 col-span-2">
                          ...
                          <div className="text-[#63e2b7] font-mono font-bold flex gap-4">
                            <span>
                              Hex/ASCII: {decodedBytes[selectedElementIndex]?.representation}
                            </span>
                            <span className="text-[10px] text-[#52607a]">
                              Raw binary: {decodedBytes[selectedElementIndex]?.rawBits}
                            </span>
                          </div>
                        </div>
                      )}
                      {el.type === "atom" && (
                        <div className="space-y-1">
                          ...
                          <div className="text-amber-400 font-bold">
                            {el.isActive ? "ON (Filled/1)" : "OFF (Shell/0)"}
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex-1 flex flex-col justify-center items-center text-[#52607a] text-xs py-4 text-center">
                ...
                <span>
                  No node selected. Click on any atom or sequence node to inspect its discrete state
                  value.
                </span>
              </div>
            )}
            ...
            <div className="h-6 border-t border-[#141822]/50 flex items-center justify-between text-[8px] font-mono text-[#52607a] pt-2">
              ...
              <span>
                TOTAL STEMS: {parsed.reduce((acc, el) => acc + (el.chars?.length || 0), 0)}
              </span>
