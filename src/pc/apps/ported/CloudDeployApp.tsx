...
export const CloudDeployApp = () => {
  const [selectedProvider, setSelectedProvider] = useState<"vercel" | "gcp" | "replit" | null>(
    null,
  );
  ...
  const handleDeploy = async () => {
    ...
    if (selectedProvider !== "replit") {
      ...
      setLogs([
        `No ${selectedProvider === "vercel" ? "Vercel" : "Google Cloud"} account is connected to this project.`,
    ...
    try {
      ...
      if (!data.success) {
        setLogs((prev) => [
          ...prev,
          "Build failed:",
          data.stderr || data.stdout || "Unknown build error",
        ]);
        setDeploying(false);
        return;
      }
      setLogs((prev) => [
        ...prev,
        `Build succeeded in ${(data.durationMs / 1000).toFixed(1)}s.`,
    ...
    <div className="h-full w-full bg-zinc-950 flex flex-col text-zinc-100 font-mono overflow-y-auto">
      <div className="p-6 border-b border-zinc-800 bg-zinc-900">
        ...
        <p className="text-sm text-zinc-400 max-w-lg">
          Deploy your Semantic Compression Pods and Global State architecture directly to your
          linked cloud providers for zero-latency universal access.
        </p>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <Box
                size={32}
                className={selectedProvider === "vercel" ? "text-zinc-100" : "text-zinc-400"}
              />
            ...
            >
              <Server
                size={32}
                className={selectedProvider === "gcp" ? "text-blue-400" : "text-zinc-400"}
              />
              <div className="text-center">
            ...
            >
              <Globe
                size={32}
                className={selectedProvider === "replit" ? "text-orange-400" : "text-zinc-400"}
              />
              <div className="text-center">
            ...
            {logs.length === 0 ? (
              ...
              <div className="space-y-2">
                {logs.map((log, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-2 ${i === logs.length - 1 && !deployed ? "text-indigo-400" : "text-zinc-400"}`}
                  >
