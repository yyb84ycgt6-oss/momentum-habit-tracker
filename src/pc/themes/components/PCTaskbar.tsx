  ...
  return (
    <>
      ...
      <div className="pc-taskbar">
        ...
        >
          <StartLogo
            kind={theme.taskbar.startLogo}
            size={theme.taskbar.startLogo === "orb" ? 20 : 17}
          />
        ...
        {/* Running windows */}
        <div
          className="flex items-center gap-[3px] min-w-0 overflow-x-auto"
            ...
            >
              <PCAppIcon
                item={{
                  ...
                  name: "Themes",
                  type: "app",
                  icon: SettingsIcon as DesktopItem["icon"],
                }}
                pack={theme.iconPack}
                size={16}
              />
...
const PCStartMenu: React.FC<PCStartMenuProps> = (props) => {
  ...
  switch (theme.startMenu) {
    case "classic":
      return <StartClassic {...props} />;
    case "luna":
      return <StartLuna {...props} />;
    case "aero":
      return <StartAero {...props} />;
    case "fullgrid":
      return <StartFullGrid {...props} />;
    case "list10":
      return <StartList10 {...props} />;
    case "centered11":
      return <StartCentered11 {...props} />;
  }
};
...
const StartClassic: React.FC<PCStartMenuProps> = ({
  apps,
  onLaunch,
  onLaunchAppId,
  onShutDown,
  ...
  return (
    <div className="pc-startmenu" style={{ width: 300, maxHeight: "68vh" }}>
      <div className="pc-startmenu-banner">
        <span>{theme.label}</span>
      </div>
      <div className="flex flex-col min-w-0 flex-1">
        <div className="overflow-y-auto flex-1" style={{ maxHeight: "calc(68vh - 110px)" }}>
          {apps.map((a) => (
            <MenuRow key={a.id} item={a} onLaunch={onLaunch} />
          ))}
        </div>
        <div
          style={{
            borderTop: "1px solid var(--pc-bevel-dark)",
            boxShadow: "0 1px 0 var(--pc-bevel-light) inset",
          }}
        >
          <button className="pc-menu-item" onClick={() => onLaunchAppId("system_settings")}>
            <PCAppIcon
              item={{ id: "", name: "Settings", type: "app" } as DesktopItem}
              pack={theme.iconPack}
              size={24}
            />
            <span>Settings</span>
          ...
          <button className="pc-menu-item" onClick={() => onLaunchAppId("pc_themes")}>
            <PCAppIcon
              item={{ id: "", name: "Display Themes", type: "app" } as DesktopItem}
              pack={theme.iconPack}
              size={24}
            />
            <span>Themes…</span>
          ...
          <button className="pc-menu-item" onClick={onShutDown}>
            <PCAppIcon
              item={{ id: "", name: "Shut Down computer", type: "app" } as DesktopItem}
              pack={theme.iconPack}
              size={24}
            />
...
const StartLuna: React.FC<PCStartMenuProps> = ({ apps, onLaunch, onLaunchAppId, onShutDown }) => {
  ...
  return (
    <div
      className="pc-startmenu flex-col"
      style={{ width: 400, maxHeight: "70vh", borderRadius: "8px 8px 0 0" }}
    >
      <div
        className="flex items-center gap-2 px-3 py-2 text-white font-bold"
        style={{
          background: "linear-gradient(180deg, var(--pc-banner-to) 0%, var(--pc-banner-from) 100%)",
        }}
      >
        <span
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div
          className="w-36 shrink-0 flex flex-col gap-1 p-2"
          style={{ background: "#d3e5fa", borderLeft: "1px solid #95bdee" }}
        >
          <button className="pc-menu-item" onClick={() => onLaunchAppId("system_settings")}>
            Control Panel
          </button>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(180deg, var(--pc-banner-from) 0%, var(--pc-banner-to) 100%)",
        }}
      >
  ...
  return (
    <div className="pc-startmenu flex-col" style={{ width: 340, maxHeight: "70vh" }}>
      <div className="overflow-y-auto flex-1 p-1.5">
        {filtered.map((a) => (
          <MenuRow key={a.id} item={a} size={28} onLaunch={onLaunch} />
        ))}
        {filtered.length === 0 && (
          <div className="px-3 py-4 text-xs opacity-60">No items match your search.</div>
        )}
      </div>
      <div
      ...
      >
        ...
        <button
          className="pc-btn"
          style={{ minWidth: 0 }}
          title="Shut down (back to Jackie)"
          onClick={onShutDown}
        >
          <Power size={13} />
        </button>
...
const StartFullGrid: React.FC<PCStartMenuProps> = ({
  apps,
  onLaunch,
  onLaunchAppId,
  onShutDown,
  ...
  <div className="pc-startmenu flex-col" style={{ padding: "18px 24px" }}>
    <div className="flex items-center justify-between mb-4 shrink-0">
      <span className="text-3xl font-light" style={{ fontFamily: "var(--pc-titlebar-font)" }}>
        Start
      </span>
      ...
      {apps.map((a) => {
        ...
        return (
          <button
            key={a.id}
            className="pc-tile"
            style={{ height: 78, background: "rgba(255,255,255,0.14)" }}
            onClick={() => onLaunch(a)}
          >
...
const StartList10: React.FC<PCStartMenuProps> = ({ apps, onLaunch, onLaunchAppId, onShutDown }) => (
  <div className="pc-startmenu" style={{ width: 460, height: "min(66vh, 520px)" }}>
    <div
      className="flex flex-col justify-end items-center gap-2 p-1.5 shrink-0"
    ...
    <div className="w-48 shrink-0 overflow-y-auto py-1">
      {apps.map((a) => (
        <MenuRow key={a.id} item={a} size={22} onLaunch={onLaunch} />
      ))}
    </div>
...
const StartCentered11: React.FC<PCStartMenuProps> = ({
  apps,
  onLaunch,
  onLaunchAppId,
  onShutDown,
  ...
  return (
    <div
      className="pc-startmenu flex-col"
      style={{ width: "min(560px, 94%)", maxHeight: "70vh", padding: 14 }}
    >
      ...
      <div
        className="flex items-center justify-between pt-3 mt-2 shrink-0"
