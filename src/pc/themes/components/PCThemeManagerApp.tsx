...
const GAME_PACK: Array<{ appId: string; title: string; blurb: string }> = [
  {
    appId: "snake",
    title: "Snake",
    blurb: "The timeless grid crawler. Our Minesweeper-era welcome classic.",
  },
  {
    appId: "chess",
    title: "Zenith Chess",
    blurb: "Strategy pack — thinks harder than Chess.exe ever did.",
  },
  {
    appId: "iron-men-arcade",
    title: "Iron Men Arcade",
    blurb: "Cabinet action in the SkiFree spirit. Watch for the yeti.",
  },
  {
    appId: "laser-tag",
    title: "Laser Tag Arcade",
    blurb: "3D Pinball energy — full tilt, no table required.",
  },
];
...
export const PCThemeManagerApp: React.FC = () => {
  ...
  const tabBtn = (t: Tab, label: string) => (
    <button
      ...
      style={{
        borderBottomColor: tab === t ? "var(--pc-accent, #6366f1)" : "transparent",
        minWidth: 0,
      }}
    >
      {label}
  ...
  return (
    <div
      ...
      style={
        !isDefault
          ? { background: "var(--pc-surface)", color: "var(--pc-surface-text)" }
          : undefined
      }
    >
      {/* Header */}
      ...
      {/* Tabs */}
      <div
        className="flex items-end gap-1 px-3 pt-2 border-b shrink-0"
        style={{ borderColor: "rgba(127,127,127,0.3)" }}
      >
        ...
        {tab === "updates" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-xs opacity-75">
              <Download size={13} />
              Theme packs available for this PC. Installing a pack applies it instantly — no restart
              required (we’ve come a long way since 1995).
            </div>
            {themes
              .filter((t) => t.id !== "cosmic-jackie")
              .map((t) => {
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${t.preview.a}, ${t.preview.b}, ${t.preview.c})`,
                      }}
                    />
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-bold">
                        {t.label} Theme Pack{" "}
                        <span className="opacity-50 font-normal">· {t.era}</span>
                      </div>
                      <div className="text-[10px] opacity-70 truncate">{t.description}</div>
                      {isInstalling && (
                        <div
                          className="pc-progress mt-1.5 h-3 rounded-sm overflow-hidden"
                          style={{ background: "rgba(127,127,127,0.25)" }}
                        >
                          <div
                            className="pc-progress-fill h-full"
                            style={{
                              width: `${progress}%`,
                              background: "var(--pc-accent, #6366f1)",
                            }}
                          />
                        </div>
                      )}
        ...
        {tab === "welcome" && (
          <div className="space-y-4 max-w-2xl">
            <div className="rounded border p-4" style={{ borderColor: "rgba(127,127,127,0.35)" }}>
              ...
              <p
                className="text-xs leading-relaxed rounded p-3"
            ...
            <div>
              ...
              >
                {GAME_PACK.map((g) => (
                  <div
                    key={g.appId}
                    className="rounded border p-3 flex gap-3"
                    style={{ borderColor: "rgba(127,127,127,0.35)" }}
                  >
