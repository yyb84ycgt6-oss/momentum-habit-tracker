...
import {
  HABIT_COLORS, ICON_KEYS, iconFor, type HabitColor,
} from "@/lib/momentum";
...
const empty: HabitDraft = {
  name: "", description: "", icon: "sparkles", color: "teal",
...
export function HabitDialog({
  ...
  async function save() {
    ...
    setSaving(true);
    try { await onSubmit(draft); onOpenChange(false); }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input id="name" value={draft.name} maxLength={60}
              onChange={(e) => setDraft({ ...draft, name: e.target.value })}
              placeholder="e.g. Morning walk" />
          </div>
          <div>
            <Label htmlFor="desc">Description</Label>
            <Textarea id="desc" value={draft.description} rows={2} maxLength={200}
          ...
          <div>
            ...
            <div className="grid grid-cols-8 gap-1.5 mt-1.5">
              {ICON_KEYS.map((k) => {
                ...
                return (
                  <button key={k} type="button" onClick={() => setDraft({ ...draft, icon: k })}
                    className={cn(
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select id="freq" value={draft.frequency}
                onChange={(e) => setDraft({ ...draft, frequency: e.target.value as "daily" | "weekly" })}
            ...
            <div>
              ...
              <Input id="target" type="number" min={1} max={20} value={draft.target_per_period}
                onChange={(e) => setDraft({ ...draft, target_per_period: Number(e.target.value) || 1 })} />
            </div>
          </div>
        </div>
