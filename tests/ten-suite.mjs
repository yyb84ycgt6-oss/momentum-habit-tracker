...
const USER = { id: '00000000-0000-4000-8000-00000000test', aud: 'authenticated', role: 'authenticated', email: 'suite@example.test', app_metadata: {}, user_metadata: {}, created_at: new Date().toISOString() };
...
async function newPage() {
    ...
    await page.route(`${SUPABASE}/**`, async (route) => {
        ...
        if (url.includes('/auth/v1/token')) {
            return route.fulfill({
                ...
                body: JSON.stringify({ access_token: 'test', token_type: 'bearer', expires_in: 3600, refresh_token: 'test', user: USER }),
            });
        }
    ...
    const handler = async (route) => {
        ...
        return route.fulfill({
            status: 200, contentType: 'application/json',
            body: JSON.stringify({ choices: [{ message: { content: `answer from ${new URL(url).host}` }, finish_reason: 'stop' }] }),
        });
    };
    ...
    await page.addInitScript((session) => {
        ...
        localStorage.setItem(
            ...
            JSON.stringify({ access_token: 'test', token_type: 'bearer', expires_at: Math.floor(Date.now() / 1000) + 3600, refresh_token: 'test', user: session }),
        );
    }, USER);
...
ok('00. The desktop renders past the auth guard', page.url().includes('/desktop'), page.url().replace(BASE, ''));
...
ok('01. Telemetry aggregates by provider with a median (not a mean)',
    !!telemetry && telemetry.calls === 3, telemetry ? `calls=${telemetry.calls} median=${telemetry.median}ms` : 'no stats');

ok('02. Parallel foundation exposes race() and judge()',
    await page.evaluate(async () => {
        const m = await import('/src/pc/lib/ai/parallel.ts');
        return typeof m.race === 'function' && typeof m.judge === 'function';
    }));

ok('03. Recorder is OFF until explicitly started (not surveillance by default)',
    (await page.evaluate(async () => (await import('/src/pc/lib/observe/recorder.ts')).isRecording())) === false);
...
ok('05. Gateway answers through the keyring', !!gatewayWorks.provider && gatewayWorks.keyed,
    `${gatewayWorks.provider} — "${gatewayWorks.text}"`);

const excluded = await page.evaluate(async () => {
    const { chat } = await import('/src/pc/lib/ai/gateway.ts');
    const res = await chat({ messages: [{ role: 'user', content: 'hi' }], excludeProviders: ['groq'] });
    return res.provider;
});
ok('06. excludeProviders keeps a forbidden provider out of the chain', excluded !== 'groq', `answered by ${excluded}`);

const excludedNamed = await page.evaluate(async () => {
    const { chat } = await import('/src/pc/lib/ai/gateway.ts');
    const res = await chat({ messages: [{ role: 'user', content: 'hi' }], model: 'groq:llama-3.3-70b-versatile', excludeProviders: ['groq'] });
    return res.provider;
});
ok('07. An exclusion cannot be routed around by naming the provider', excludedNamed !== 'groq', `answered by ${excludedNamed}`);
...
ok('08. getAiClient (the ~25 legacy apps) reaches a real provider', /answer from/.test(shim), shim.slice(0, 40));
...
const shimShapes = await page.evaluate(async () => {
    ...
    const b = await c.models.generateContent({ contents: [{ role: 'user', parts: [{ text: 'array of content' }] }] });
    const d = await c.models.generateContent({ contents: { role: 'user', parts: [{ text: 'single object' }] } });
...
ok('10. aiClient.sendMessage (5 more apps) reaches a real provider',
    /answer from/.test(legacy.content), `${legacy.provider}`);
...
ok('11. Cortex matches a reworded question (normalized, not exact)',
...
ok('12. Offline, Cortex answers from memory and admits it is cached',
    offline.fromCache === true && offline.text.includes('isolated'), `fromCache=${offline.fromCache}`);

ok('13. Online, a fresh answer is NOT labelled as cached',
    (await page.evaluate(async () => (await import('/src/pc/lib/ai/cortex.ts')).askWithCortex('Name one colour').then((r) => r.fromCache))) === false);

// ── The Understudy ───────────────────────────────────────────────────────
ok('14. Understudy is opt-in — learning about someone is not a default',
    (await page.evaluate(async () => (await import('/src/pc/lib/understudy/predictor.ts')).isEnabled())) === false);

const learned = await page.evaluate(async () => {
    const m = await import('/src/pc/lib/understudy/predictor.ts');
    m.resetModel();
    for (let i = 0; i < 4; i += 1) { m.record('cortex'); m.record('speed_racer'); m.record('cartographer'); }
    const p = m.predictNext('cortex');
    return { top: p[0]?.appId ?? null, conf: p[0]?.confidence ?? 0, reason: p[0]?.reason ?? '' };
});
ok('15. Understudy predicts the app that actually follows',
    learned.top === 'speed_racer' && learned.conf > 0.3,
    `${learned.top} @ ${Math.round(learned.conf * 100)}% — ${learned.reason}`);

ok('16. One data point yields no prediction (guessing is not predicting)',
    (await page.evaluate(async () => {
        const m = await import('/src/pc/lib/understudy/predictor.ts');
        m.resetModel(); m.record('cortex'); m.record('speed_racer');
...
const routine = await page.evaluate(async () => {
    const m = await import('/src/pc/lib/understudy/predictor.ts');
    m.resetModel();
    for (let i = 0; i < 3; i += 1) { m.record('cortex'); m.record('speed_racer'); m.record('cartographer'); }
    const r = m.routines();
...
ok('18. Understudy scores its own predictions', scored !== null && scored > 0, `accuracy=${scored}`);

ok('19. Registry apps expose a memoized preload so chunks can be warmed',
    (await page.evaluate(async () => {
...
ok('20. A widget position round-trips through storage',
    widget.back?.x === 120 && widget.back?.y === 240, JSON.stringify(widget.back));
ok('21. An off-screen position is clamped back inside the viewport',
    widget.clamped.x < 1400 && widget.clamped.y < 950, JSON.stringify(widget.clamped));
ok('22. Reset clears every widget at once', widget.cleared === null);

const draggable = await page.locator('[title*="drag to move" i]').count();
ok('23. The desktop search affordance is wrapped as a movable widget', draggable > 0, `matched ${draggable}`);

// ── All eleven render ────────────────────────────────────────────────────
const ELEVEN = [
    ['ai_providers', 'AI Providers'], ['budget_radar', 'Budget Radar'], ['colosseum', 'Colosseum'],
    ['ambient_agents', 'Ambient Agents'], ['bus_recorder', 'Bus Recorder'], ['choreography', 'Choreograph'],
    ['speed_racer', 'Speed Racer'], ['cartographer', 'Cartographer'], ['prompt_genome', 'Genome'],
    ['cortex', 'Offline Cortex'], ['understudy', 'Understudy'],
];
const blank = [];
...
ok('24. Launching each new app renders it', blank.length === 0,
    blank.length ? `blank: ${blank.join(', ')}` : `${ELEVEN.length} of ${ELEVEN.length}`);

const registered = await page.evaluate(async () => {
    const { APPS } = await import('/src/pc/apps/registry.ts');
    const ids = ['ai_providers', 'budget_radar', 'colosseum', 'ambient_agents', 'bus_recorder',
        'choreography', 'speed_racer', 'cartographer', 'prompt_genome', 'cortex', 'understudy'];
    return { missing: ids.filter((id) => !APPS.some((a) => a.id === id)), total: APPS.length };
});
ok('25. All eleven are in the registry', registered.missing.length === 0,
    registered.missing.length ? `missing: ${registered.missing.join(', ')}` : `${registered.total} apps total`);
